...
const GroupIcons: React.ReactNode[] = [
  // Agents / multi-agent
  <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 text-[var(--sec)] opacity-70">
            ...
            >
                ...
                <div className="flex items-center gap-2 flex-grow justify-between">
                  ...
                  <svg
                    ...
                    className={`w-6 h-6 text-[var(--white)] transition-transform flex-shrink-0 ${
                      openItem === group.title ? "rotate-180" : ""
                    }`}
                  >
              ...
              <div
                className={`transition-all duration-300 px-4 ${
                  openItem === group.title ? "max-h-[500px] pb-4 opacity-100" : "max-h-0 opacity-0"
                }`}
              >
                <ul className="space-y-2 text-[var(--white-icon)] text-sm">
                  {group.items.map((item) => (
                    <li key={item} className="flex items-start gap-2">
                      <span className="text-[var(--sec)] leading-6">•</span>
                      <span>{item}</span>
                    </li>
                  ))}
                </ul>
